...
function solve_QP_ip(Hk, dfk, dgk, gk; xl=nothing, xu=nothing)
    ...
    #build C and d for dgk'p >= -gk
    C = copy(dgk)          # (n × m_ineq)
    d = -copy(gk)          # (m_ineq,)

    # add box constraints as inequalities if present
    if xl !== nothing
        finite = .!isinf.(xl)
        if any(finite)
...
function SQP_trust_region(func, ineq, x0, z0, tr0; 
    ...
    while niter < MAX_ITER
        ...
        xl_p = fill(-trk, length(xk))
        xu_p = fill(trk, length(xk))
        ...
        gkn = norm(min.(gk, 0), 1)
        if gkn > 0
            lk = (dfk' * pk + 0.5 * pk' * Hk * pk) / ((1.0 - rho) * gkn) + l0
        end

        # BFGS update
        if step_accepted && H === nothing && niter < MAX_ITER
            sk = xk1 - xk
            qk = dL_func(xk1, zk1, df_eval, dg_eval) - dL_func(xk, zk, df_eval, dg_eval) 
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    m_fixed = 3
    n_fixed = 5
    m_range = collect(1:1:8)

    println("=== Sweeping n (m=$m_fixed) ===")
    res_n = benchmark_sqp_vs_n(n_range, m_ineq=m_fixed, n_trials=3)

    println("=== Sweeping m (n=$n_fixed) ===")
